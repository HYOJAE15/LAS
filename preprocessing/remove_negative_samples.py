...
"""
This script removes all the negative samples from the dataset.
    negative samples: ones that have a label that is not all 0.
...
def loop_through_images(image_list: list):
    ...
    for image_path in image_list:
        ...
        if check_negative_sample(label_path):
            os.remove(image_path)
            os.remove(label_path)

def check_negative_sample(label_path: str):
    """
    Args:
        label_path: path to the label

    Returns:
        True if the label is all 0
    """
